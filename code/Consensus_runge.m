%Ultizzo il metodo di Runge per risolvere l'IVP
clear all; close all;
%numero droni
n=4;
...
M=A{index}(:,:);
    ...
    % stato.
    %(usiamo eulero in avanti per stimare la yn+1/2 che andremo poi a mettere
    %all'interno della formula per Runge)
    for r=1:1:n
        increment=zeros(m,m);
        ...
        NewLData(:,:,r)=exp_SO(pr,(step/2).*increment);    
    end
    ...
    for r=1:1:n 
        ...
        pr=LData(:,:,r);
        NewLData1(:,:,r)=exp_SO(pr,step.*pt_SO(pr1,pr,increment1));
